...
from examples.data_source_base import DataSource
...
class OPUSDataset(DataSource):
    ...
    def __init__(self, domain: str, split: str, src_lang: str, tgt_lang: str,
        ...
        source_texts, target_texts = self._load_translation_pairs(domain, firstn)
    ...
    def _maybe_download_unzip(self, domain_label: str) -> Tuple[str, str]:
        ...
        out_srcs = [os.path.join(self.data_dir, fpath) for fpath in os.listdir(self.data_dir)
                    if domain_label.lower() in fpath and src_suffix in fpath]
        out_tgts = [os.path.join(self.data_dir, fpath) for fpath in os.listdir(self.data_dir)
                    if domain_label.lower() in fpath and tgt_suffix in fpath]

        # resources are not yet downloaded
        if not out_srcs or not out_tgts:
            print("Downloading %s" % domain_label)
            url = OPUS_RESOURCES_URLS[domain_label] % self.ordered_lang_pair
            resp = urlopen(url)
